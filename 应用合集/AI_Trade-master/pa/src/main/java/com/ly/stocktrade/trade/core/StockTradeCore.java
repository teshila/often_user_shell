package com.ly.stocktrade.trade.core;
...
public class StockTradeCore {
	...
	public final static String GET_WEI_TUO_URL = "https://m.stock.pingan.com/pah5trade/trade/stock/orders?random=RANDOM&action_in=2&start_date=START_DATE&end_date=END_DATE";
	...
	public String doBuy(String stockCodeStr, String weiTuoPrice, Account ac, Map lastestMap) {
		String random = String.format("%.17f", Math.random());
		Map mapInfos = this.getEnableBuyNum(stockCodeStr, weiTuoPrice, ac, lastestMap);
		int num = (int) mapInfos.get("total");
		
		
		/*logger.debug("当前可买数量===> " + num  + "   买入半仓数量为 "+ CalcUtil.formatDouble(num/2.0) + "，买入1/4仓位为"+ CalcUtil.formatDouble(num/4.0));
		String buyNum ="1400.00";
		if(num>2400){
			if((num/100)%2==0){
				buyNum= CalcUtil.formatDouble(num/4.0);//默认买入1/4仓
			}else{
				buyNum= CalcUtil.formatDouble((num-100)/4.0);//默认买入1/4仓
			}
			buyNum = buyNum.substring(0,buyNum.length()-3);
		}*/
		...
		logger.debug("当前可买数量===> " + num  + "   买入半仓数量为 "+ changWei2 + "   买入1/3仓数量为 "+ changWei3 + "，买入1/4仓位为"+changWei4);
		
		int calcbuySum = changWei3;
		if(num>=8000){
			calcbuySum = changWei5;
		}else if(num>=1600){
			calcbuySum = changWei4;
		}
	...
	public int doSell(String stockCodeStr, String weiTuoPrice, Account ac, Map lastestMap) {
		String random = String.format("%.17f", Math.random());
	...
	// 从委托表中查询需要撤单的-------暂时不需要
	/*public int doCheDanByList(Map<String,String> paramMap, Account ac) {
		...
		if (list != null && list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
				...
				if (code.equals(stockCode)) {
					...
					headers.put("X-Requested-With", "XMLHttpRequest");
					String result = httpReq.httpPost(url, headers, paraMap);
					logger.debug("【代码】" + stockCode + ",返回值代码 2，撤单数据返回结果  " + result);
					return 2;
				} else {
					logger.debug("【代码】" + code+ "无相关委托信息,手动委托的,返回值代码 3");
					returnInfo = 3;
				}

			}
		} else {
			logger.debug("【代码】" + code + "无相关委托信息,返回值代码 3");
			returnInfo = 3;
		}
