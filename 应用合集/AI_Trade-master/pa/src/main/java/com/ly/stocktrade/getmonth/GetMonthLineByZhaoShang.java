...
@Component
public class GetMonthLineByZhaoShang {
	
	private static Logger logger = Logger.getLogger(GetMonthLineByZhaoShang.class);
	...
	private static final String ZHAO_SHANG_WEEK_URL = "https://hq.cmschina.com/market/json?funcno=20033&version=1&stock_list=CODE&type=month&count=15&date=DATE&timeStamp=TIMESTAMP";//count 50 为最近50周
	
	
	public List  getMonthLineInfo(String code,String name){
		Date date = new Date();
		...
		String url = ZHAO_SHANG_WEEK_URL.replace("CODE", tempCode).replace("DATE", dateStr).replace("TIMESTAMP", String.valueOf(timeStampLong));
