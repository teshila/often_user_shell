package com.ly.stocktrade.getdaylinema;
...
@Component
public class GetDayLineByZhaoShangMa {
	
	private static Logger logger = Logger.getLogger(GetDayLineByZhaoShangMa.class);
	...
	public List  getDayLineInfo(String code,String name){
		Date date = new Date();
		...
		*ST，是连续三年亏损，有退市风险的意思，购买这样的股票要有比较好的基本面分析能力。
		...
		if(result!=null){
			try {
				...
				logger.info("招商证券返回的数据======> " +  weekInfoList);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
