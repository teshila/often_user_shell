...
import com.ly.stocktrade.getmonth.GetMonthLineByZhaoShang;
import com.ly.task.getmonthline.byauto.MonthLineCalcByByAutoService;

@Component
public class GetMonthLineInfoByZhaoShangTask {
	private static Logger logger = Logger.getLogger("month");

	@Autowired
	private StockDao stockDao;
	@Autowired
	private GetMonthLineByZhaoShang getWeekLineByZhaoShang;
	@Autowired
	...
	// @Scheduled(cron="*/10 * * * * ?")
	// @Scheduled(cron="30 54 19 * * ?")
	//@Scheduled(cron = "10 11 16 ? * MON,WED,FRI")
	//@Scheduled(cron="30 38 21 * * ?")
	@Scheduled(cron = "10 11 16 ? * MON,WED,FRI")
	public void task() throws InterruptedException {
		logger.info("开始获取月线数据=====>采集开始");
		...
		for (int i = 1; i <= step; i++) {
			...
			if (sbLists != null && sbLists.size() > 0) {
				for (int j = 0; j < sbLists.size(); j++) {
					 ...
					 */
					sb = sbLists.get(j);
					logger.info("需要获取月线数据的股票代码【 " + sb.getCode() + " 】，名称 【" + sb.getName() + "】,所属市场编码 【 " + sb.getMarketType() + " 】");
					List resultList = getWeekLineByZhaoShang.getMonthLineInfo(sb.getCode(), sb.getName());
