...
//@Component
public class GetWeekLineInfoByTencentTask {
		...
		for (int i = 1; i <= step; i++) {
			...
			if (sbLists != null && sbLists.size() > 0) {
				for (int j = 0; j < sbLists.size(); j++) {
					...
					week.info("当前爬虫到第" + (i + 1) + "条数据,共" + sbLists.size() + "条数据，还有" + (sbLists.size() - (i + 1)) + "条数据没爬虫到，当前代码【" + sb.getCode() + "】,名称【" + sb.getName() + "】返回信息" + resultList);
