...
public class StockShouYangTwoDayController {
	...
	public Page getInfo(String pageNum,String pageSize){
		...
		int total  = (int) stock_Shou_Yang_One_DayDao.findCount(Stock_Shou_Yang_Two_Day.class);
	...
	public Map findBlockDay(String code) {
		...
		if(StringUtils.isNotEmpty(code)){
			...
			if(MyStringUtil.isInteger(code)){
				lists = stock_Shou_Yang_One_DayDao.findByPage("from Stock_Shou_Yang_Two_Day t where t.code  like '%'||?||'%' ",0,20,code);
			}else{
				lists = stock_Shou_Yang_One_DayDao.findByPage("from Stock_Shou_Yang_Two_Day t where t.pinyin  like '%'||?||'%'  ",0,20,code);
			}
			ret.put("list", lists);
		}else{
			ret.put("msg", "代码不能为空");
