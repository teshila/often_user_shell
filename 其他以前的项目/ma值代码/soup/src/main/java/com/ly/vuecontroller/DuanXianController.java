...
public class DuanXianController {
	...
	@RequestMapping("/getZhouShouYangTime")
	public Map getMaxDay(String code) {
		Map ret = new HashMap();
		//List<Stock_Shou_Yang_Day_And_Week_Shou_Yang> time = stock_Shou_Yang_One_DayDao.find("select max(time) from  Stock_Shou_Yang_Day_And_Week_Shou_Yang");
	...
	@RequestMapping("/findZhouShouYang")
	public Map findBlockDay(String code) {
		Map ret = new HashMap();
		if(StringUtils.isNotEmpty(code)){
			List lists = null;
			if(MyStringUtil.isInteger(code)){
				lists = stock_Shou_Yang_One_DayDao.find("from Stock_Shou_Yang_Day_And_Week_Shou_Yang t where t.code  like '%'||?||'%' ",code);
			}else{
				lists = stock_Shou_Yang_One_DayDao.find("from Stock_Shou_Yang_Day_And_Week_Shou_Yang t where t.pinyin  like '%'||?||'%'  ",code);
			}
			ret.put("list", lists);
		}else{
			ret.put("msg", "代码不能为空");
