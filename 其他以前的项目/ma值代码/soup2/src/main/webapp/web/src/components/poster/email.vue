						...
						<div class="container-fluid">
							<div class="row clearfix">
								<div class="col-md-3 column systemInfo">
									...
									【<span @click="getSendLog(account.address)" :class="account.times>0?'red':''">{{account.address}}</span>】
									<div class="alert alert-danger hide">
										   <button type="button" class="close" @click="closeLog(account.address)">&times;</button>
								...
								<div class="col-md-3 column opts">
									操作： 【
									<span @click="del(account.address,index)">删除</span>】 【
									<span @click="setSend(account.address,index)">设置发送</span>】 【
									<span @click="setNotSend(account.address,index)">取消发送</span>】
								</div>
							</div>
						</div>
					</li>
			...
			<div class="panel-body">
				...
				<ul class="list-group" id="lists">
					<li class="list-group-item" v-for="(account,index) in senders">
						<div class="container-fluid">
							<div class="row clearfix">
								<div class="col-md-3 column opts">邮箱地址：【<span @click="addSender(account.address,account.port,account.protocol,account.host,account.pwd)">{{account.address}}</span>】</div>
								<div class="col-md-3 column">
									端口：【{{account.port }}】， 协议：【{{account.protocol}}】， 服务器地址：【{{account.host}}】， 默认发信人【{{account.flag>0?"是":"否"}}】
								</div>
								<div class="col-md-3 column opts">
									操作： 【
									<span @click="delSender(account.address,index)">删除</span>】 【
									<span @click="setDefaultSender(account.address,index)">设置为默认发信人</span>】 
...
<script>
	...
	export default {
		...
		methods: {
			...
			delSender: function(address, index) {
				MessageBox.confirm('确定执行此删除操作?').then(action => {
			...
			addSender: function(address, port, protocol, host, pwd) {
				this.isshow = true;
				var tempObj = {};
			...
			setDefaultSender: function(address, index) {
			...
			setNotDefaultSender: function(address, index) {
				MessageBox.confirm('确定设置发送吗?').then(action => {
					var url = this.baseURL.apisafe + '/updateDefaultSender.do?rand=' + Math.random();
					var token = this.cookie.getCookie("token");
					var params = new URLSearchParams();
			...
			getSendLog: function(address) {
					...
					params.append('isSend', "0");
					this.$ajax.post(url, params).then((response) => {
						var list = response.data.pageList;
						if(list!=null){
...
// WEBPACK FOOTER //
// src/components/poster/email.vue