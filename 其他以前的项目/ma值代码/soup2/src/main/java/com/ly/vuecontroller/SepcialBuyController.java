...
public class SepcialBuyController {
	...
	public Page<SpecialBuy> getInfo(String pageNum,String pageSize){
		int pageNo = 0;
		int row = 10;
		...
		List<SpecialBuy> list = buyDao.findByPage("from SpecialBuy ORDER BY addtime desc", pageNo, row);
		int total  = (int) buyDao.findCount(SpecialBuy.class);
		
		Page<SpecialBuy> pages = new Page<SpecialBuy>();
	...
	public Map addAccount(SpecialBuy accWeb){
		Map<String,String> map = new HashMap<String,String>();
		if(accWeb!=null&&accWeb.getCode()!=null&&StringUtils.isNotEmpty(accWeb.getCode())){
			try {
				String code =  accWeb.getCode();
				List<SpecialBuy> list  = buyDao.find("from SpecialBuy t where t.code = ?",code);
				if(list!=null&&list.size()>0){
					SpecialBuy ac  = list.get(0);
	...
	public Map deleBuyInfo(SpecialBuy buy) {
		Map ret = new HashMap();
	...
	public Map truncateBuy() {
		Map ret = new HashMap();
		try {
			buyDao.truncateTable(SpecialBuy.class);
			ret.put("msg", "删除成功");
		} catch (Exception e) {
