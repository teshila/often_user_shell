...
@Component
public class Chi_CangDayUpComputing {
	
	private static Logger calday = Logger.getLogger(Chi_CangDayUpComputing.class);
	@Autowired
	...
	public void getStockInfo() {
		...
		for (int i = 1; i <= step; i++) {
			...
			for (int j = 0; j < list.size(); j++) {
				...
				List<Stock> stockList = stockDao.find("from Stock", code);
				...
				if(stockList!=null&&stockList.size()>0){
					for (int k = 0; k < stockList.size(); k++) {
						...
						//	if(day1IsZhang>0&&day1IsBigThanMa5>0&&day1Ma5IsBigThanyesterMa5>0&&day2IsZhang>0&&day2IsBigThanMa5>0&&day2Ma5IsBigThanyesterMa5>0){
							if(day1IsZhang>0&&day2IsZhang>0){
