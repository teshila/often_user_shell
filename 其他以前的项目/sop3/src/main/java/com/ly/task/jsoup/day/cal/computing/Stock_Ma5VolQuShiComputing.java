...
//短线算法
@Component
public class Stock_Ma5VolQuShiComputing {
	...
	@Autowired
	private Stock_Ma5VolQuShiDao stock_Ma10_DaYu_Ma20Dao;
	
	//@Scheduled(cron="0 34 2 ? * *")
	//@Scheduled(cron = "40 05 16 ? * MON-FRI")
	//
	//@Scheduled(cron="*/10 * * * * ?")
	//@Scheduled(cron="30 20 16 ? * *")
	public void getStockInfo() {
		long startTime = System.currentTimeMillis();    //获取开始时间
		List<Stock_Ma5VolQuShi> entitys =  null;
		stock_Ma10_DaYu_Ma20Dao.truncateTable(Stock_Ma5VolQuShi.class);
		Stock_Ma5VolQuShi stock_Shou_Yang_Day_One_Day = null;
		...
		for (int i = 1; i <= step; i++) {
			entitys = new ArrayList<Stock_Ma5VolQuShi>(); 
			...
			for (int j = 0; j < list.size(); j++) {
				...
				if(datas!=null&&datas.size()>=20){
					...
					for(int arr =0;arr<15;arr++){
						...
						double day3IsZhang= day2Open - day3Open;
					
					   double day1Ma10IsBigDay2Ma10 = day1Ma5Vol -day2Ma5Vol;
					...
					if(day1IsZhang>0&&day1IsBigThanMa5>0&&day1Ma5IsBigThanyesterMa5>0&&day1Ma10IsBigDay2Ma10>=0&&ma5VolIsZhang>0){
						...
						stock_Shou_Yang_Day_One_Day = new Stock_Ma5VolQuShi();
