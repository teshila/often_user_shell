...
public class StockShouYangTwoDayController {
	...
	public Page getInfo(String pageNum,String pageSize){
		...
		List list = stock_Shou_Yang_One_DayDao.findByPage("from Stock_Shou_Yang_Two_Day "+SystemConfig.myAppendStr, pageNo, row);
		List totalList  =  stock_Shou_Yang_One_DayDao.find("select count(1) from  Stock_Shou_Yang_Two_Day "+SystemConfig.condition);
	...
	public Map getMaxDay(String code) {
		Map ret = new HashMap();
		String time = (String)stock_Shou_Yang_One_DayDao.getCurrentSession().createQuery("select max(time) from  Stock_Shou_Yang_Two_Day").list().get(0);
		ret.put("time", time);
		return ret;
	}
	...
	public Page<Stock_Shou_Yang_Two_Day> findBlockDay(String code) {
		...
		if(StringUtils.isNotEmpty(code)){
			if(MyStringUtil.isInteger(code)){
				list = stock_Shou_Yang_One_DayDao.findByPage("from Stock_Shou_Yang_Two_Day t where t.code  like '%'||?||'%' ",0,20,code);
				countList  =  stock_Shou_Yang_One_DayDao.find("select count(1) from  Stock_Shou_Yang_Two_Day t where t.code  like '%'||?||'%' ",code);
				total = (long)countList.get(0);
			}else{
				list = stock_Shou_Yang_One_DayDao.findByPage("from Stock_Shou_Yang_Two_Day t where t.pinyin  like '%'||?||'%' ",0,20,code);
				countList  =  stock_Shou_Yang_One_DayDao.find("select count(1) from  Stock_Shou_Yang_Two_Day t where t.pinyin like '%'||?||'%' ",code);
				total = (long)countList.get(0);
