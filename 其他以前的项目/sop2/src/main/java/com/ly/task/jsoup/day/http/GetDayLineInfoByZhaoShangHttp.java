...
@Component
public class GetDayLineInfoByZhaoShangHttp {
	...
	private static final String ZHAO_SHANG_DAY_URL = "https://hq.cmschina.com/market/json?funcno=20033&version=1&stock_list=CODE&type=day&count=30&date=DATE&timeStamp=TIMESTAMP";//count 50 为最近50周
	
	private static final String ZHAO_SHANG_DAY_URL_REQ = "https://hq.cmschina.com/market/json?funcno=20036&version=1&market=SZ&stock_code=000983&type=day&Lastcount=&count=1&timeStamp=1549103674535";//count 50 为最近50周
	
	
	public List  getDayLineInfo(String code,String name) throws Exception{
		Date date = new Date();
		...
		*ST，是连续三年亏损，有退市风险的意思，购买这样的股票要有比较好的基本面分析能力。
		...
		logger.info("处理形成的股票对像数据" + stockJsonStr);
	...
	public String httpRequest01(String code,String name,String tempCode,String stockJsonStr,String dateStr,String timeStampLong ) throws Exception{
		String url = ZHAO_SHANG_DAY_URL.replace("CODE", tempCode).replace("DATE", dateStr).replace("TIMESTAMP", String.valueOf(timeStampLong));
		...
		logger.info("获取从招商证券日线信息req01  === >" + url);
	...
	public String httpRequest02(String code,String name,String tempCode,String stockJsonStr,String market,String dateStr,String timeStampLong ) throws Exception{
		String url = ZHAO_SHANG_DAY_URL_REQ.replace("CODE", code).replace("MARKET", market).replace("DATE", dateStr).replace("TIMESTAMP", timeStampLong);
		Map<String,String> headers = new HashMap<String,String>();
		headers.put("accept", "application/json, text/javascript, */*; q=0.01");
		headers.put("Accept-Encoding", "gzip, deflate, br");
		headers.put("Accept-Language", "zh-CN,zh;q=0.9");
		...
		headers.put("X-Requested-With", "XMLHttpRequest");
		String result = zhaoShangHttp.httpGet(url, headers);
		logger.info("request02  获取从招商证券中周线信息的URL  === >" + url);
		logger.info("招商证券中周线===> " + result);
		return result;
	}
