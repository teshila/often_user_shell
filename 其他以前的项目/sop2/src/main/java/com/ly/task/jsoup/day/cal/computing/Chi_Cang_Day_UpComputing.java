...
@Component
public class Chi_Cang_Day_UpComputing {
	
	//https://blog.csdn.net/c694421919/article/details/82287626  git 文件冲突解决
	
	private static Logger calday = Logger.getLogger(Chi_Cang_Day_UpComputing.class);
	@Autowired
	...
	public void getStockInfo() {
		...
		List<Chi_Cang_Day_Up> entitys =  null;
		stock_Shou_Yang_One_DayDao.truncateTable(Chi_Cang_Day_Up.class);
		Chi_Cang_Day_Up stock_Shou_Yang_Day_One_Day = null;
		long total = chiCangDao.findCount(Chi_Cang.class);
		int rows = SystemSoupConstrant.rows;
		int seconds = SystemSoupConstrant.seconds;
		int step = (int) (total / rows + 1);
		
		for (int i = 1; i <= step; i++) {//一定要从1开始，否则会出现重复
			entitys = new ArrayList<Chi_Cang_Day_Up>(); 
			List<Chi_Cang> list = chiCangDao.findByPage("from Chi_Cang", i, rows);
			
			for (int j = 0; j < list.size(); j++) {
				Chi_Cang chi_Cang = list.get(j);
				String code = chi_Cang.getCode();
				
				List<Stock> stockList = stockDao.find("from Stock s where s.code = ? ", code);
				...
				if(stockList!=null&&stockList.size()>0){
					for (int k = 0; k < stockList.size(); k++) {
						...
						//	if(day1IsZhang>0&&day1IsBigThanMa5>0&&day1Ma5IsBigThanyesterMa5>0&&day2IsZhang>0&&day2IsBigThanMa5>0&&day2Ma5IsBigThanyesterMa5>0){
							//if(day1IsZhang>0&&day2IsZhang>0&&day2IsBigThanMa5>0&&day1Ma5IsBigThanyesterMa5>0){
							if(day1IsZhang>0){
								stock_Shou_Yang_Day_One_Day = new Chi_Cang_Day_Up();
