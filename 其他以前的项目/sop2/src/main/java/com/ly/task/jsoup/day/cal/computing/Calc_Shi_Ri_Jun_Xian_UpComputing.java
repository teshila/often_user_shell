...
@Component
public class Calc_Shi_Ri_Jun_Xian_UpComputing {
	
	private static Logger calday = Logger.getLogger("calcShiRi");
	...
	//@Scheduled(cron="50 52 22 ? * *")
	public void getStockInfo() {
		long startTime = System.currentTimeMillis();    //获取开始时间
		List<Stock_Shi_Ri_Jun_Xian_Up> entitys =  null;
		shi_Ri_Jun_Xian_UpDao.truncateTable(Stock_Shi_Ri_Jun_Xian_Up.class);
		Stock_Shi_Ri_Jun_Xian_Up stock_Shou_Yang_Day_One_Day = null;
		...
		for (int i = 1; i <= step; i++) {
			entitys = new ArrayList<Stock_Shi_Ri_Jun_Xian_Up>(); 
			...
			for (int j = 0; j < list.size(); j++) {
				...
				if(datas!=null&&datas.size()>=3){
					...
					Double day10Ma5 = Double.valueOf(day3.getMa10());
					...
					calday.debug("五日线 "+ wuRiJunXianUp + " ,十日线 "+shiRiJunXianUp+" ===>" + sts.getCode() + "  ==> "+ sts.getName());
					calday.info("五日线 "+ wuRiJunXianUp + " ,十日线 "+shiRiJunXianUp+" ===>" + sts.getCode() + "  ==> "+ sts.getName());
					
					if(shiRiJunXianUp>=0&&wuRiJunXianUp>0&&wuRiIsBigThanShiRi>=0&&day1IsBigThanMa10>0&&day1IsBigThanMa5>0&&openIsBig>0){
						
						stock_Shou_Yang_Day_One_Day = new Stock_Shi_Ri_Jun_Xian_Up();
			...
			shi_Ri_Jun_Xian_UpDao.batchSave(entitys);
